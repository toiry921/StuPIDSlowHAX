...
static bool stupidslowhax_kernel11_backdoor(KSemaphore *this, void *thread)
{
	g_backdoor_method();
	return true;
}

static void stupidslowhax_kernel11_setup_step1(KSemaphore *this)
{
	// Turn interrupts off
...
static void stupidslowhax_kernel11_setup_step2(void)
{
	// Turn interrupts off
	__asm__ volatile("cpsid aif");
...
static void stupidslowhax_setRefCount(Handle handle, u32 value)
{
	s64 outInfo;
...
bool stupidslowhax_run(void)
{
	...
	res = svcCreateSemaphoreKAddr(&sVtable, 0,
		(u32)stupidslowhax_kernel11_setup_step1, &kObject);
	printf("Creating KSemaphore: %08lx h%08lx @%08lx\n", res, sVtable,
		(u32)kObject);
	...
	if(g_debug_mode)
		...
		stupidslowhax_setRefCount(sHax, 0U);
	...
	g_fake_ksemaphore_vtable[12] = stupidslowhax_kernel11_backdoor;


	// Restore "hax" KSemaphore data
	stupidslowhax_backdoor(stupidslowhax_kernel11_setup_step2);
